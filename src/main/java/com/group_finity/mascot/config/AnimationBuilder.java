...
public class AnimationBuilder {
    ...
    private Pose loadPose(final Entry frameNode) throws IOException {
        String imageText = frameNode.getAttribute(schema.getString("Image")) != null ? imageSet + frameNode.getAttribute(schema.getString("Image")) : null;
        if (imageText != null) {
            imageText = System.getProperty("user.dir") + "/img" + imageText;
            imageText = imageText.replace("\\", "/");
        }
        String imageRightText = frameNode.getAttribute(schema.getString("ImageRight")) != null ? imageSet + frameNode.getAttribute(schema.getString("ImageRight")) : null;
        if (imageRightText != null) {
            imageRightText = System.getProperty("user.dir") + "/img" + imageRightText;
            imageRightText = imageRightText.replace("\\", "/");
        }
        ...
        if (imageText != null) {
            try {
                ...
                ImagePairLoader.load(
                        imageText,
                        imageRightText,
                        anchor, scaling, filter, opacity);
            } catch (Exception e) {
